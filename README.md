...
In order to choose the best model, Gridsearch and hyper-parameter tuning were used for five classifiers: XGBoost, Random Forest, KNN, Logistic Regression and Decision Tree . Based on comparisons of the accuracy score,  XGBoost classifier was chosen for the final model.
...
I worked with four different data-frames with more than 30,000 rows.
...
My final data frame before feature engineering was a concatenation of 4 tables and had 11,260 rows and 12 columns. 
...
The model with the best result after Hyper-parameter Tuning  used the *XGBoost Classifier* and reached 0.733 Accuracy
...
It is challenging and not very accurate to predict Income based only on Social Security payments and Geography in a zip code. Nevertheless, some general first impressions can be concluded:
...
With additional data (for example tax collection) and with application on all the zip code in the US (this model include about one third of them) more accurate model can be built, based on similar methods that were used in this project.
